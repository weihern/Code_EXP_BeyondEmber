...
import { updateDoc, doc, collection, getDocs, onSnapshot, query, terminate, arrayUnion } from "firebase/firestore";
            ...
            <View
              style={[
                item.difficulty === "Hard"
                  ...
                  ? MainStyles.moderateDiv
                  : MainStyles.easyDiv,
                { borderRadius: 50, padding: 5, position: "absolute", right: remToDp(3), top: remToDp(3) },
              ]}
            >
...
export function Challenge3({ navigation, route }) {
  ...
  // Handle like and dislike accordingly
  const handleLikeDislike = async (index,like) => {
    if (like){
      data.suggestions[index].like += 1;
    } else {
      data.suggestions[index].dislike += 1;
    }

    const newsuggest = data;
    const docRef = doc(db, "challenges", id);
    if (!voted){
      try {
        await updateDoc(docRef, {
          suggestions: newsuggest.suggestions,
        });
        // update locally as snapshot is not declare in challenge3
        const updatedsuggest = [...suggestions];
        setSuggestions(updatedsuggest)
        setVoted(true)
      } catch (error) {
        console.error("Error updating like to Firestore:", error);
      }
      let toast = Toast.show("Voted Successfully", {
      ...
      let toast = Toast.show("User has already voted", {
        duration: Toast.durations.LONG,
        backgroundColor: "#BB86FC", // Set the background color here
        textColor: "white",
        fontFamily: "righteous",
  ...
  const rowRenderer = ({ item, index }) => (
    <View
      key={index}
      style={[
        MainStyles.containerPrimary,
        { backgroundColor: "#D9D9D9", justifyContent: "space-between", flexDirection: "row" },
      ]}
    >
      ...
      <View
        style={{ display: "flex", flexDirection: "row", padding: 5, alignItems: "center", justifyContent: "center" }}
      >
        <View style={{ alignItems: "center", justifyContent: "center", marginEnd: 5 }}>
          <TouchableOpacity onPress={() => handleLikeDislike(index,true)}>
            <Icon name="like" />
        ...
        <View style={{ alignItems: "center", justifyContent: "center", marginEnd: 5 }}>
          <TouchableOpacity onPress={() => handleLikeDislike(index,false)}>
            <Icon name="dislike" />
      ...
      {MainStyles && (
        <View style={[MainStyles.container, { justifyContent: "start" }, inputFocus && { height: "58%" }]}>
          <View
            style={[
              MainStyles.containerPrimary,
              { width: "90%", paddingHorizontal: remToDp(3), paddingTop: remToDp(3) },
            ]}
          >
            <Text style={[MainStyles.textHeader, { color: "#BDBDBD", fontSize: 16, marginBottom: remToDp(2) }]}>
              {user} Posted
            </Text>
          ...
          <View
            style={[
              MainStyles.containerPrimary,
              { backgroundColor: "#F5F5F5", bottom: 20, position: "absolute", width: "90%", padding: 10 },
              MainStyles.shadowDiv,
            ]}
          >
            <TextInput
              value={inputText}
